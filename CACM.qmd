# Designometric user experience and the psychometric fallacy

In modern industrial practice, user experience rating scales have their place as an always available and inexpensive method for comparing or benchmarking designs. In times of CD/CI this became even more important. In this dynamic decision process, the everyday value of a rating scales stands and falls with two properties: validity and reliability.

After the landrush phase of the UX revolution, Bargas-Avila & Hoernbaek ((Old wine in new bottles)) counted hundreds of new rating scale instruments. Few to none of these first generation instrument were developed to the standards that an instrument for psycho-diagnostics would have to. Still, many authors provided at least some proof of quality for the instruments by using psychometric techniques as they are routinely used in various fields of social sciences.

*Developing* a valid and reliable rating scale is quite an undertaking. *Psychometrics* originally is the science of assigning numbers to persons for the purpose of ranking by level of cognitive functioning, for example mathematical intelligence or comprehension of language. Item-response models are routinely used in high-stake tests such as the international school comparison studies TIMSS and PISA.

Modern psychometrics also includes the alternative factor-analytic approach, which is more common in clinical diagnostics, as well as social psychology. While the premise for psychometrics test is purely a valid ranking, factor-analytic approaches emphasize latent structures in behavior.

Both approaches have many concepts in common: First, they both measure up against validity and reliability. A test is said to be valid, when it has predictive value, whereas reliability is concerned with how efficient the instrument is. \
Second, and crucially, both approaches rest on the idea that an (unobservable) latent variable can be observed indirectly through items that provoke a response. Because individual items are noisy, many items are used to improve accuracy of measurements. When many persons do a test, the produced data is traditionally written in the form of item-response matrices as shown in \[FIG\]. And practically all psychometric tools take person-by-item matrices as input. It is more than reasonable to transfer long tested psychometric tools for the purpose of good UX rating scales, \*if\* it is done right.

The purpose of a UX rating scale is to rank designs by some relevant property, such as Beauty. In analogy, they should be called *designometric scales*. The capability to predict which design leaves a good impression defines validity and reliability of a designometric scale.

The development of psychometric scales usually requires immense participant sample sizes, for two reasons: First, the sample must sufficiently cover the target population. Second, psychometric models carry many parameters and are therefore data greedy. Similar to how multiple items are used to accurately assess persons, many persons are needed to assess individual items during scale development.

If we transfer these general idea to designometrics perspective, designometric item consistency and other methods must be applied on design-by-item response matrices and scale development must involve a *large sample of designs* and a sample of users.

In practice, many designometric rating scales have been developed using a large participant sample and one or a few designs at best. From a purely logical perspective, the capability to rank designs cannot be assessed on a single design.

A basic but representative psychometric method is to assess item-level consistency as the correlation between item score and total score of a scale.
